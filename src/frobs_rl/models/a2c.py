...
class A2C(basic_model.BasicModel):
    ...
    def __init__(
        ...
        config_filename="a2c_config.yaml",
        ns="/",
    ) -> None:
        """
        A2C constructor.
        ...
        if load_trained:
            ...
            self.model = stable_baselines3.A2C.load(il_policy_path, env=env)
            ...
            model_learning_rate = rospy.get_param(
                ns + "/model_params/a2c_params/learning_rate"
            )
            ...
            model_gae_lambda = rospy.get_param(
                ns + "/model_params/a2c_params/gae_lambda"
            )
            ...
            model_max_grad_norm = rospy.get_param(
                ns + "/model_params/a2c_params/max_grad_norm"
            )
            model_use_rms_prop = rospy.get_param(
                ns + "/model_params/a2c_params/use_rms_prop"
            )
            model_rms_prop_eps = rospy.get_param(
                ns + "/model_params/a2c_params/rms_prop_eps"
            )
            model_norm_advant = rospy.get_param(
                ns + "/model_params/a2c_params/normalize_advantage"
            )
            ...
            if rospy.get_param(ns + "/model_params/load_model"):  # Load model
                ...
                self.model = stable_baselines3.A2C.load(
